...
Worker 检测到名为 `EMAIL`、支持 `send()` 的邮件绑定后，会同时开启注册邮箱验证和忘记密码入口。新账号必须在 30 分钟内打开单次验证链接；密码重置链接 20 分钟有效，并会撤销旧会话。未配置绑定时，`/api/capabilities` 会明确返回两个能力均为 `false`，前端不会展示失效入口。

Cloudflare 账户必须先开通支持向任意注册邮箱投递的 Email Sending 服务，再将其绑定为 `EMAIL`。不要用仅允许发送到预先验证地址的测试绑定冒充生产邮件服务，也不要把邮件供应商令牌写入 `wrangler.jsonc` 或提交到 Git。
