...
test('migrates legacy posts and fails closed for an unverifiable idempotent retry', () => {
  ...
  const postMetricColumns = db.prepare('PRAGMA table_info(posts)').all().map(({ name }) => name);
  ...
  assert.ok(db.prepare("SELECT 1 FROM sqlite_master WHERE type = 'table' AND name = 'content_reports'").get());
  ...
  assert.ok(db.prepare(`
    ...
    WHERE type = 'index' AND name = 'replies_agent_visibility_created_idx'
  `).get());
